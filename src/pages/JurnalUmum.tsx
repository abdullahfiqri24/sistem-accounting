...
import { JOURNALS, COST_CENTERS } from '@/lib/data'
import { fmtIDR, fmtDate } from '@/lib/utils'
import type { Journal } from '@/types'

function JournalSlideOver({ journal, onClose, onAction }: {
  journal: Journal
  onClose: () => void
      ...
      <div className="anim-slidein-right" style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0, fontSize: 12 }}>
            {auditTrail.map((ev, i, arr) => (
              <div key={i} style={{ display: 'flex', gap: 12, paddingBottom: 12, position: 'relative' }}>
                <div style={{ width: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
                  <div style={{
                    width: 8, height: 8, borderRadius: '50%',
                    background: ev.kind === 'approve' ? 'var(--positive)' : ev.kind === 'void' ? 'var(--negative)' : ev.kind === 'wait' ? 'var(--status-pending)' : 'var(--brand-maroon)',
        ...
        <div style={{ padding: '12px 20px', borderTop: '1px solid var(--border)', background: 'var(--bg-subtle)', display: 'flex', gap: 8, justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            <button className="btn btn-sm" onClick={onClose}>Tutup <kbd className="kbd">esc</kbd></button>
          </div>
        </div>
      </div>
    </>
...
export default function JurnalUmum() {
  ...
  const [filters, setFilters] = useState({ status: 'all', cc: 'all', q: '' })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ padding: '16px 24px 12px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button className="btn btn-primary btn-sm" onClick={() => navigate('/jurnal-baru')}>
              <Icon name="plus" size={13}/> Buat Jurnal <kbd className="kbd">N</kbd>
            </button>
          </div>
        </div>
        <Tabs
      ...
      <div style={{ padding: '10px 24px', display: 'flex', gap: 8, alignItems: 'center', borderBottom: '1px solid var(--border)', background: 'var(--bg-subtle)' }}>
        ...
        <select className="input input-sm select" style={{ width: 'auto' }} value={filters.cc} onChange={(e) => setFilters({ ...filters, cc: e.target.value })}>
      ...
      {selected.size > 0 && (
        <div className="anim-fadein" style={{ padding: '8px 24px', background: 'rgba(91,2,2,0.06)', borderBottom: '1px solid var(--border)', display: 'flex', gap: 8, alignItems: 'center', fontSize: 12.5 }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', position: 'relative' }}>
        <table className="tbl">
          ...
          <tbody>
            {filtered.map((j) => (
              <tr key={j.id} className={selected.has(j.id) ? 'selected' : ''} onClick={() => setDetail(j)} style={{ cursor: 'pointer' }}>
      ...
      <div style={{ padding: '8px 24px', borderTop: '1px solid var(--border)', background: 'var(--bg-elevated)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12, color: 'var(--fg-tertiary)' }}>
