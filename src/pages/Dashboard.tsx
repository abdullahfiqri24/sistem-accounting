...
import { DASHBOARD_KPI, PIUTANG_JT, UTANG_JT, PENDING_APPROVAL, REVENUE_TREND, PENDAPATAN_UNIT } from '@/lib/data'
import { fmtIDR, fmtIDRShort, fmtDate } from '@/lib/utils'
...
function AgingPill({ days }: { days: number }) {
  ...
  return <span className="badge num" style={{ color, background: bg, fontSize: 10.5 }}>{days} hari</span>
}

function Kpi({ label, value, delta, sub, icon, accent }: {
  label: string; value: number; delta: number; sub: string; icon: string; accent: 'maroon' | 'gold'
}) {
  return (
    ...
    <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <button className="btn btn-sm" onClick={() => navigate('/kas-masuk-baru')}>
            <Icon name="plus" size={13}/> Kas Masuk
          </button>
          <button className="btn btn-sm" onClick={() => navigate('/kas-keluar-baru')}>
            <Icon name="plus" size={13}/> Kas Keluar
          </button>
          <button className="btn btn-sm" onClick={() => navigate('/jurnal-baru')}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        <Kpi label="Saldo Kas" value={DASHBOARD_KPI.kas.value} delta={DASHBOARD_KPI.kas.delta} sub={DASHBOARD_KPI.kas.sub} icon="plus" accent="gold"/>
        ...
        <div className="card" style={{ padding: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
            <svg width="120" height="120" viewBox="0 0 120 120" style={{ flexShrink: 0 }}>
              ...
              <text x="60" y="69" textAnchor="middle" style={{ fontSize: 11, fontWeight: 700, fill: 'var(--fg)', fontFamily: 'var(--font-mono)' }}>
          ...
          <div style={{ padding: '12px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border)' }}>
            <div>
              ...
              <div style={{ fontSize: 11.5, color: 'var(--fg-tertiary)', marginTop: 2 }}>
                {PENDING_APPROVAL.length} jurnal menunggu — total {fmtIDRShort(PENDING_APPROVAL.reduce((s, x) => s + x.amount, 0))}
              </div>
            </div>
            <a onClick={() => navigate('/jurnal-umum')} style={{ fontSize: 11.5, color: 'var(--brand-maroon)', cursor: 'pointer', fontWeight: 600 }}>Lihat semua →</a>
          </div>
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ padding: '12px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border)' }}>
            <div>
              ...
              <div style={{ fontSize: 11.5, color: 'var(--fg-tertiary)', marginTop: 2 }}>
                {PIUTANG_JT.length} invoice · total {fmtIDRShort(PIUTANG_JT.reduce((s, x) => s + x.amount, 0))}
              </div>
            </div>
            <a onClick={() => navigate('/piutang')} style={{ fontSize: 11.5, color: 'var(--brand-maroon)', cursor: 'pointer', fontWeight: 600 }}>Lihat semua →</a>
          </div>
          <div style={{ overflowX: 'auto' }}>
            <table className="tbl compact">
              <thead>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div className="card" style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ padding: '12px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border)' }}>
            <div>
              ...
              <div style={{ fontSize: 11.5, color: 'var(--fg-tertiary)', marginTop: 2 }}>
                {UTANG_JT.length} PO · total {fmtIDRShort(UTANG_JT.reduce((s, x) => s + x.amount, 0))}
              </div>
            </div>
            <a onClick={() => navigate('/utang')} style={{ fontSize: 11.5, color: 'var(--brand-maroon)', cursor: 'pointer', fontWeight: 600 }}>Lihat semua →</a>
          </div>
          <div style={{ overflowX: 'auto' }}>
            <table className="tbl compact">
              <thead>
              ...
              <tbody>
                {UTANG_JT.map((u) => (
                  <tr key={u.id}>
                    ...
                    <td>
                      <AgingPill days={u.aging}/>
                      <span style={{ marginLeft: 6, fontSize: 11, color: 'var(--fg-tertiary)' }}>{fmtDate(u.due)}</span>
                    </td>
        ...
        <div className="card" style={{ padding: 16 }}>
          ...
          <div style={{ marginTop: 12, padding: 10, background: 'var(--status-pending-bg)', color: 'var(--status-pending)', borderRadius: 6, display: 'flex', gap: 8, alignItems: 'flex-start', fontSize: 12 }}>
            ...
            <span>
              Rekonsiliasi BSI Payroll belum dilakukan untuk minggu lalu.{' '}
              <a onClick={() => navigate('/kas-masuk')} style={{ fontWeight: 600, cursor: 'pointer', textDecoration: 'underline' }}>Buka rekonsiliasi →</a>
            </span>
          </div>
        </div>
      </div>
