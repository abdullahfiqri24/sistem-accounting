...
const Login = ({ onLogin }) => {
  const [username, setUsername] = React.useState("akt.dewi");
  const [password, setPassword] = React.useState("•••••••••");
  const [showPw, setShowPw] = React.useState(false);
  const [role, setRole] = React.useState("akuntan");
    ...
    <div className="login-bg" style={{
      ...
      <div className="login-brand-panel" style={{ padding: "48px 56px", display: "flex", flexDirection: "column", justifyContent: "space-between", position: "relative" }}>
        ...
        <div style={{ maxWidth: 440 }}>
          ...
          <h1 style={{ fontSize: 36, lineHeight: 1.15, margin: "0 0 14px", fontWeight: 700, letterSpacing: "-0.025em" }}>
            Pencatatan keuangan rumah sakit yang <span style={{ color: "#daa520" }}>cepat, rapi, auditable</span>.
          </h1>
      ...
      <div className="login-form-panel" style={{
        ...
        <form onSubmit={submit} style={{ width: "100%", maxWidth: 380 }}>
          {/* Mobile-only brand */}
          <div style={{ display: "none", alignItems: "center", gap: 12, marginBottom: 26 }} className="login-mobile-brand">
            <div style={{
              width: 40, height: 40, borderRadius: 8,
              background: "var(--brand-maroon)",
              display: "flex", alignItems: "center", justifyContent: "center",
            }}>
              <svg width="22" height="22" viewBox="0 0 24 24"><path d="M9.5 4h5v4H18v5h-3.5V18h-5v-5H6v-5h3.5z" fill="var(--brand-gold)"/></svg>
            </div>
            <div>
              <div style={{ fontSize: 10.5, color: "var(--fg-tertiary)", letterSpacing: "0.18em", fontWeight: 600, textTransform: "uppercase" }}>RS YARSI</div>
              <div style={{ fontSize: 15, fontWeight: 700, letterSpacing: "-0.01em" }}>Sistem Akuntansi Keuangan</div>
            </div>
          </div>
          <style>{`@media (max-width: 1024px) { .login-mobile-brand { display: flex !important; } }`}</style>
          ...
          <div style={{ marginBottom: 14 }}>
            <label className="field-label" style={{ display: "flex", justifyContent: "space-between" }}>
              <span>Password</span>
              <a href="#" onClick={(e) => e.preventDefault()} style={{ color: "var(--brand-maroon)", textDecoration: "none", fontSize: 11, textTransform: "none", letterSpacing: 0 }}>Lupa password?</a>
            </label>
            <div style={{ position: "relative" }}>
