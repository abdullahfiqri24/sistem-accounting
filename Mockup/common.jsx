...
const Icon = ({ name, size = 16, className = "", strokeWidth = 1.75, style = {} }) => {
  ...
  switch (name) {
    ...
    case "arrow-down-circle": return <svg {...props}><circle cx="12" cy="12" r="9"/><path d="M8 12l4 4 4-4"/><path d="M12 8v8"/></svg>;
    case "arrow-up-circle": return <svg {...props}><circle cx="12" cy="12" r="9"/><path d="M16 12l-4-4-4 4"/><path d="M12 16V8"/></svg>;
    ...
    case "scale": return <svg {...props}><path d="M16 16l3-8 3 8c-2 1-4 1-6 0"/><path d="M2 16l3-8 3 8c-2 1-4 1-6 0"/><path d="M7 21h10"/><path d="M12 3v18"/><path d="M3 7h18"/></svg>;
    ...
    case "settings": return <svg {...props}><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09a1.65 1.65 0 0 0-1-1.51 1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09a1.65 1.65 0 0 0 1.51-1 1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9c0 .67.4 1.26 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>;
    ...
    case "eye-off": return <svg {...props}><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94"/><path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19"/><path d="m1 1 22 22"/><path d="M9 9a3 3 0 0 0 4.24 4.24"/></svg>;
    ...
    case "tree": return <svg {...props}><path d="M3 5h18M3 12h18M3 19h18"/></svg>;
    ...
    case "info": return <svg {...props}><circle cx="12" cy="12" r="9"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>;
    ...
    case "logo-cross": return (
      <svg width={s} height={s} viewBox="0 0 24 24" className={className} style={style}>
        <rect x="2" y="2" width="20" height="20" rx="4" fill="var(--brand-maroon)"/>
        <path d="M9.5 6h5v3.5H18v5h-3.5V18h-5v-3.5H6v-5h3.5z" fill="var(--brand-gold)"/>
      </svg>
    );
    default: return null;
  }
  ...
  return (
    <ToastContext.Provider value={{ push }}>
      ...
      <div style={{ position: "fixed", bottom: 16, right: 16, zIndex: 100, display: "flex", flexDirection: "column", gap: 8, pointerEvents: "none" }}>
        {toasts.map((t) => (
          <div key={t.id} className="anim-fadein" style={{
            background: "var(--bg-elevated)",
            border: `1px solid var(--border-strong)`,
            borderLeft: `3px solid var(--${t.kind === "error" ? "negative" : t.kind === "success" ? "positive" : "brand-maroon"})`,
            borderRadius: 6,
            padding: "10px 14px",
            fontSize: 12.5,
            minWidth: 260,
...
const Tabs = ({ tabs, value, onChange }) => (
  <div style={{ display: "flex", gap: 4, borderBottom: "1px solid var(--border)", marginBottom: 0 }}>
    {tabs.map((t) => {
      ...
      return (
        <button key={t.value} className="btn-ghost" onClick={() => onChange(t.value)}
          style={{
...
const EmptyState = ({ title, subtitle, action }) => (
  <div style={{ padding: "60px 20px", textAlign: "center", color: "var(--fg-tertiary)" }}>
    <div style={{ fontSize: 14, fontWeight: 600, color: "var(--fg-secondary)", marginBottom: 6 }}>{title}</div>
    <div style={{ fontSize: 13, marginBottom: 16 }}>{subtitle}</div>
    {action}
  </div>
);

Object.assign(window, { Icon, StatusBadge, Kbd, ToastProvider, useToast, Tabs, EmptyState });
