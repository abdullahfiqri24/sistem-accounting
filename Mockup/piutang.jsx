...
const Piutang = ({ pageMode = "all", onNav }) => {
  const { PIUTANG, fmtIDR, fmtDate } = window.YarsiData;
  const { push } = useToast();
  const [tab, setTab] = React.useState(pageMode === "all" ? "all" : pageMode.replace("piutang-", ""));
  const [agingFilter, setAgingFilter] = React.useState("all");
  const [detail, setDetail] = React.useState(null);

  React.useEffect(() => {
    if (pageMode !== "all" && pageMode.startsWith("piutang-")) {
      setTab(pageMode.replace("piutang-", ""));
    }
  }, [pageMode]);
  ...
  const agingPill = (days) => {
    ...
    return <span className={`badge no-dot num ${cls}`} style={{ fontSize: 10.5 }}>{days} hari</span>;
  };
          ...
          return (
            <button key={b.key}
              ...
              <div className="num" style={{ fontSize: 17, fontWeight: 700, marginTop: 4, letterSpacing: "-0.01em" }}>
                {fmtIDR(buckets[b.key], { decimals: 0 })}
              </div>
              <div style={{ fontSize: 11, color: "var(--fg-tertiary)", marginTop: 2 }}>
                {totalOutstanding > 0 ? ((buckets[b.key] / totalOutstanding) * 100).toFixed(1) : 0}% dari total
              </div>
            </button>
          );
        })}
...
const PiutangSlideOver = ({ invoice, onClose, onPay }) => {
  ...
  const breakdown = invoice.episode.includes("Rawat Inap") ? [
    { item: "Akomodasi Kamar (3 malam)", qty: 3, harga: 800000, total: 2400000 },
    { item: "Visite Dokter Spesialis", qty: 6, harga: 250000, total: 1500000 },
    { item: "Tindakan Medis & Operasi", qty: 1, harga: invoice.tagihan * 0.45, total: invoice.tagihan * 0.45 },
    { item: "Farmasi & Alkes", qty: 1, harga: invoice.tagihan * 0.28, total: invoice.tagihan * 0.28 },
    { item: "Laboratorium & Radiologi", qty: 1, harga: invoice.tagihan * 0.12, total: invoice.tagihan * 0.12 },
    { item: "Lain-lain", qty: 1, harga: invoice.tagihan - 3900000 - invoice.tagihan * 0.85, total: Math.max(invoice.tagihan - 3900000 - invoice.tagihan * 0.85, 200000) },
  ] : [
    { item: "Konsultasi Dokter", qty: 1, harga: invoice.tagihan * 0.3, total: invoice.tagihan * 0.3 },
    { item: "Pemeriksaan", qty: 1, harga: invoice.tagihan * 0.5, total: invoice.tagihan * 0.5 },
    { item: "Farmasi", qty: 1, harga: invoice.tagihan * 0.2, total: invoice.tagihan * 0.2 },
  ];

  return (
    <>
      <div onClick={onClose} className="anim-fadein" style={{ position: "fixed", inset: 0, background: "rgba(10,12,16,0.32)", zIndex: 40 }}/>
      <div className="anim-slidein-right slide-over-panel" style={{
        ...
        <div style={{ padding: "14px 20px", borderBottom: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ fontSize: 11.5, color: "var(--fg-tertiary)", marginTop: 1 }}>
              {invoice.payer} · {fmtDate(invoice.tgl)}
            </div>
          </div>
